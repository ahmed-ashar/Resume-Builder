...
const EditResume = () => {
  ...
  // Update a simple nested object (like profileInfo, contactInfo, etc.)
  ...
  // Update array item object (like workExperience[0], skills[1] etc.)
  const updateArrayItem = (section, index, key, value) => {
    setResumeData((prev) => {
      const updateArray = [...prev[section]];

      if (key === null) {
        updateArray[index] = value; // for simple strings like in `interests`
      } else {
        updateArray[index] = {
          ...updateArray[index],
          [key]: value,
  ...
  return (
    <DashboardLayout>
      ...
      >
        <div className="w-[98vw] h-[90vh]" ref={resumeDownloadRef}>
          <RenderResume
            templateId={resumeData?.template?.theme || ""}
            resumeData={resumeData}
            colorPalatte={resumeData?.template?.colorPalatte || []}
